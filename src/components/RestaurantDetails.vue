<template>
          ...
          <tr v-for="rest in restaurentDetails" v-bind:key="rest.RestoId">
            ...
            <td > 
              ...
              <button type="button" class="btn btn light mr-1">
                <router-link :to="`/UpdateRestaurant/`+rest.RestoId"><i class="fa-regular fa-pen-to-square"></i></router-link>
              </button>
              <button v-on:click="deleteResto(rest.RestoId)" type="button" class="btn btn light mr-1">
                <i class="fa-solid fa-trash-can"></i>
              </button>
            </div>
            </td>
            <td>
              <button type="button" class="btn btn-dark mb-1" ><router-link to="/FoodDetails">Available Food</router-link></button>
  ...
  <script>
    export default {
      ...
      deleteResto(RestoId){
        console.warn(RestoId)
        // fetch(`http://localhost:5250/api/Restaurent/${RestoId}`, {
        // method: "DELETE"
        // })
